...
/* ── Racing car engine sound ─────────────────────────────────────────────
   Must be called INSIDE a user-gesture handler (click) to satisfy
   browser autoplay policy — AudioContext will be in 'running' state.
   ─────────────────────────────────────────────────────────────────────── */
function playEngineSound() {
  try {
    const ctx = new (window.AudioContext || window.webkitAudioContext)()

    // Master volume — much louder than before
    ...
    // ── 1. Ignition CLICK (loud sharp noise burst) ────────────────────
    ...
    clickGain.gain.value = 2.0
    clickSrc.connect(clickGain)
    clickGain.connect(master)
    clickSrc.start(now)

    // ── 2. Starter motor crank (rhythmic pulsing noise) ───────────────
    const crankBuf = ctx.createBuffer(1, Math.floor(ctx.sampleRate * 0.55), ctx.sampleRate)
    const ckd = crankBuf.getChannelData(0)
    const pulseHz = 18 // cranking cycles per second
    for (let i = 0; i < ckd.length; i++) {
      const t = i / ctx.sampleRate
      const pulse = Math.sin(2 * Math.PI * pulseHz * t) > 0 ? 1.0 : 0.15
      ckd[i] = (Math.random() * 2 - 1) * pulse * 0.9
    }
    const crankSrc = ctx.createBufferSource()
    crankSrc.buffer = crankBuf
    const crankBpf = ctx.createBiquadFilter()
    crankBpf.type = 'bandpass'
    crankBpf.frequency.value = 160
    crankBpf.Q.value = 0.8
    const crankGain = ctx.createGain()
    crankGain.gain.setValueAtTime(0, now + 0.1)
    crankGain.gain.linearRampToValueAtTime(1.4, now + 0.2)
    crankGain.gain.linearRampToValueAtTime(0, now + 0.65)
    crankSrc.connect(crankBpf)
    crankBpf.connect(crankGain)
    crankGain.connect(master)
    crankSrc.start(now + 0.1)

    // ── 3. Engine fires — sawtooth oscillator stack ───────────────────
    // Fundamental + 2nd + 3rd harmonic, each with own gain
    const oscDefs = [
      { freq: 52,  type: 'sawtooth', gain: 1.0,  detune: 0   },
      { freq: 104, type: 'sawtooth', gain: 0.55, detune: 8   },
      { freq: 156, type: 'square',   gain: 0.35, detune: -6  },
    ]

    oscDefs.forEach(({ freq, type, gain: gVal, detune }) => {
      const osc = ctx.createOscillator()
      osc.type = type
      osc.detune.value = detune

      // Freq sweep: low idle → mid → scream → settle
      osc.frequency.setValueAtTime(freq * 0.55, now + 0.55)
      osc.frequency.linearRampToValueAtTime(freq * 1.0,  now + 1.0)
      osc.frequency.linearRampToValueAtTime(freq * 2.6,  now + 2.0)
      osc.frequency.linearRampToValueAtTime(freq * 4.5,  now + 2.7)
      osc.frequency.linearRampToValueAtTime(freq * 3.8,  now + 3.3)

      // Low-pass opens up as engine revs
      const lpf = ctx.createBiquadFilter()
      lpf.type = 'lowpass'
      lpf.Q.value = 3.0
      lpf.frequency.setValueAtTime(250,  now + 0.55)
      lpf.frequency.linearRampToValueAtTime(900,  now + 1.2)
      lpf.frequency.linearRampToValueAtTime(3500, now + 2.7)
      lpf.frequency.linearRampToValueAtTime(2200, now + 3.3)

      const oscGain = ctx.createGain()
      oscGain.gain.setValueAtTime(0,    now + 0.5)
      oscGain.gain.linearRampToValueAtTime(gVal, now + 0.85)
    ...
    // ── 4. Rumble layer — filtered white noise ────────────────────────
    const rumbleLen = Math.floor(ctx.sampleRate * 4.0)
    ...
    rLpf.type = 'lowpass'
    rLpf.frequency.setValueAtTime(90,  now + 0.55)
    rLpf.frequency.linearRampToValueAtTime(500, now + 2.5)
    const rGain = ctx.createGain()
    rGain.gain.setValueAtTime(0,    now + 0.55)
    rGain.gain.linearRampToValueAtTime(0.6, now + 1.1)
    rSrc.connect(rLpf)
    rLpf.connect(rGain)
    rGain.connect(master)
    rSrc.start(now + 0.55)

    // ── 5. Exhaust POPS at peak rev ───────────────────────────────────
    ;[2.55, 2.72, 2.88].forEach(t => {
      ...
      for (let i = 0; i < pd.length; i++) {
        pd[i] = (Math.random() * 2 - 1) * Math.exp(-i / (ctx.sampleRate * 0.018))
      }
      const pSrc = ctx.createBufferSource()
      pSrc.buffer = pBuf
      const pGain = ctx.createGain()
      pGain.gain.value = 1.8
      pSrc.connect(pGain)
...
export default function LoadingScreen({ onDone }) {
  ...
  const [started, setStarted]       = useState(false)   // user clicked
  const quote = QUOTES[Math.floor(Math.random() * QUOTES.length)]

  // Called when user clicks the "tap to start" overlay
  const handleStart = () => {
    if (started) return
    setStarted(true)
    playEngineSound()   // ← called directly inside click handler = valid user gesture
  }

  // Thunder flash effect — only starts after user has clicked
  useEffect(() => {
    if (!started) return
    const flashTimes = [400, 1200, 2100]
    const timers = flashTimes.map(t =>
      setTimeout(() => {
        setFlashActive(true)
  ...
  // Auto-dismiss after 4s from when user clicked
  useEffect(() => {
    if (!started) return
    const timer = setTimeout(() => {
      if (onDone) onDone()       // tell parent first — app becomes visible
      setVisible(false)          // then unmount loading screen
    }, 4000)
  ...
  return (
    <div
      ...
      aria-label="Loading ShopEasy"
      role="status"
      onClick={handleStart}
      style={{ cursor: started ? 'default' : 'pointer' }}
    >

      {/* Tap-to-start prompt — disappears once clicked */}
      {!started && (
