...
export default function Cart() {
  ...
  if (items.length === 0) return (
    <div className="container empty-state" style={{ minHeight:'75vh' }}>
      <motion.span initial={{ scale:0 }} animate={{ scale:1 }} transition={{ type:'spring', bounce:0.5 }} style={{ fontSize:64 }}>🛒</motion.span>
    ...
    <div className="container" style={{ padding:'clamp(20px,4vw,40px) clamp(16px,4vw,40px) 64px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr clamp(280px,30vw,360px)', gap:24, alignItems:'start' }} className="cart-grid">
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <AnimatePresence>
            {items.map((item, idx) => (
              <motion.div key={item.id}
                ...
                style={{ background:'rgba(8,2,35,0.82)', backdropFilter:'blur(14px)', border:'1px solid rgba(245,200,66,0.14)', borderRadius:16, padding:16, display:'flex', gap:14, alignItems:'center' }}
              >
                <Link to={`/product/${item.id}`} style={{ flexShrink:0 }}>
                  <img src={item.image} alt={item.title} style={{ width:72, height:72, objectFit:'cover', borderRadius:10, border:'1px solid rgba(245,200,66,0.12)' }} />
                </Link>
                <div style={{ flex:1, minWidth:0 }}>
                  <Link to={`/product/${item.id}`} style={{ textDecoration:'none' }}>
                    <p style={{ fontSize:14, fontWeight:600, color:'rgba(240,232,255,0.9)', marginBottom:3, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden', lineHeight:1.4 }}>{item.title}</p>
                  </Link>
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      ...
                      <motion.button whileHover={{ scale:1.1 }} whileTap={{ scale:0.9 }}
                        ...
                        style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:8, width:30, height:30, display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(239,68,68,0.8)', fontSize:14, cursor:'pointer' }}
                      >✕</motion.button>
                    </div>
                  </div>
                </div>
          ...
          <button onClick={clearCart} className="btn btn-outline btn-sm" style={{ alignSelf:'flex-start', color:'rgba(239,68,68,0.8)', borderColor:'rgba(239,68,68,0.2)' }}>🗑 Clear Cart</button>
        </div>
        ...
        >
          ...
          {[['Subtotal',`₹${total.toLocaleString('en-IN')}`],['Delivery',delivery===0?'FREE':`₹${delivery}`]].map(([l,v])=>(
            <div key={l} style={{ display:'flex', justifyContent:'space-between', fontSize:14, color:'rgba(200,180,255,0.7)', marginBottom:10 }}>
          ...
          {total <= 999 && <p style={{ fontSize:12.5, color:'rgba(245,200,66,0.75)', background:'rgba(245,200,66,0.08)', border:'1px solid rgba(245,200,66,0.15)', borderRadius:8, padding:'8px 12px', marginBottom:14, textAlign:'center' }}>Add ₹{(1000-total).toLocaleString('en-IN')} more for free delivery!</p>}
          {total > 999 && <p style={{ fontSize:12.5, color:'rgba(34,197,94,0.85)', background:'rgba(34,197,94,0.08)', border:'1px solid rgba(34,197,94,0.2)', borderRadius:8, padding:'8px 12px', marginBottom:14, textAlign:'center' }}>🎉 Free delivery unlocked!</p>}
