  ...
  if (placed) return (
    <div className="container empty-state" style={{ minHeight:'80vh' }}>
      ...
      <motion.p initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.45 }}
        style={{ color:'rgba(200,180,255,0.7)', maxWidth:360, lineHeight:1.7 }}>
        Thank you for shopping with ShopEasy. You'll receive a confirmation shortly.
      </motion.p>
    ...
    <div className="container" style={{ padding:'clamp(20px,4vw,40px) clamp(16px,4vw,40px) 64px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr clamp(280px,30vw,360px)', gap:24, alignItems:'start' }} className="checkout-grid">
        ...
        <motion.form initial={{ opacity:0, x:-20 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.1 }}
          onSubmit={handleOrder} style={{ display:'flex', flexDirection:'column', gap:16 }}
        >
          {/* Delivery */}
          <div style={{ background:'rgba(8,2,35,0.85)', backdropFilter:'blur(16px)', border:'1px solid rgba(245,200,66,0.15)', borderRadius:18, padding:24 }}>
            <h2 style={{ fontSize:16, fontWeight:700, color:'var(--gold)', marginBottom:20, display:'flex', alignItems:'center', gap:8 }}>
              📦 Delivery Details
            </h2>
              ...
              {FIELDS.map(f => (
                <div key={f.key} className="form-group">
                  ...
                  <input type={f.type} placeholder={f.placeholder} value={form[f.key]}
                    ...
                    style={{ background:'rgba(255,255,255,0.05)', border:'1.5px solid rgba(245,200,66,0.16)', borderRadius:10, padding:'12px 14px', fontSize:14, color:'#fff', outline:'none', width:'100%', transition:'all 0.2s' }}
                  />
                </div>
              ))}
            </div>
          ...
          {/* Payment */}
          <div style={{ background:'rgba(8,2,35,0.85)', backdropFilter:'blur(16px)', border:'1px solid rgba(245,200,66,0.15)', borderRadius:18, padding:24 }}>
            <h2 style={{ fontSize:16, fontWeight:700, color:'var(--gold)', marginBottom:18, display:'flex', alignItems:'center', gap:8 }}>
              💳 Payment Method
            </h2>
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {PAYMENT_OPTIONS.map(p => (
                <motion.label key={p.value} whileHover={{ scale:1.01 }}
                  style={{
                    display:'flex', alignItems:'center', gap:12, padding:'13px 16px',
                    background: form.payment===p.value ? 'rgba(245,200,66,0.1)' : 'rgba(255,255,255,0.04)',
                    border: `1.5px solid ${form.payment===p.value ? 'rgba(245,200,66,0.45)' : 'rgba(255,255,255,0.08)'}`,
                    borderRadius:12, cursor:'pointer', fontSize:14, fontWeight:500,
                    color: form.payment===p.value ? 'var(--gold)' : 'rgba(220,200,255,0.8)',
                    transition:'all 0.15s',
                  }}
                >
                  <input type="radio" name="payment" value={p.value} checked={form.payment===p.value}
                    onChange={() => update('payment', p.value)}
          ...
          <motion.button whileHover={{ scale:1.02 }} whileTap={{ scale:0.98 }}
            ...
            style={{
              ...
              color:'#0a0020', fontSize:16, fontWeight:800, cursor: loading?'not-allowed':'pointer',
              boxShadow:'0 8px 30px rgba(245,200,66,0.35)', transition:'all 0.2s',
        ...
        >
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:0, maxHeight:260, overflowY:'auto', marginBottom:16 }}>
            {items.map(i => (
