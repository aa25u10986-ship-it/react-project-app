...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    getProductById(id)
      .then(res => { setProduct(res.data); setRelated(res.related||[]); setLoading(false) })
  ...
  if (loading) return (
    <div className="container empty-state" style={{ minHeight:'70vh' }}>
      ...
      <p style={{ color:'rgba(200,180,255,0.6)' }}>Loading...</p>
    </div>
  )

  if (error || !product) return (
    <div className="container empty-state" style={{ minHeight:'70vh' }}>
      <span className="icon">😕</span><h2>Product not found</h2>
  ...
  return (
    <div className="container" style={{ paddingTop:32, paddingBottom:64 }}>
      ...
      >
        ...
        <span>/</span><span style={{ color:'rgba(220,200,255,0.8)', fontWeight:500 }}>{product.title}</span>
      </motion.nav>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,340px),1fr))', gap:40, marginBottom:56, alignItems:'start' }}>
        <motion.div initial={{ opacity:0, x:-30 }} animate={{ opacity:1, x:0 }} transition={{ duration:0.55 }}>
          <div style={{ borderRadius:20, overflow:'hidden', background:'rgba(8,2,35,0.6)', border:'1px solid rgba(245,200,66,0.15)', aspectRatio:'1' }}>
        ...
        >
          ...
          <h1 style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(22px,4vw,34px)', fontWeight:800, lineHeight:1.15, color:'#fff' }}>{product.title}</h1>
          ...
          <div>
            ...
            <p style={{ fontSize:12, color:'rgba(34,197,94,0.85)', marginTop:5, fontWeight:600 }}>✓ In Stock · Free delivery over ₹999</p>
          </div>
          ...
          <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
            <motion.button whileHover={{ scale:1.03 }} whileTap={{ scale:0.97 }} onClick={handleAdd}
              style={{ flex:1, minWidth:140, padding:'14px 20px', borderRadius:12, border:'none', background:added?'linear-gradient(135deg,#22c55e,#16a34a)':'linear-gradient(135deg,var(--gold),var(--gold-d))', color:'#0a0020', fontSize:15, fontWeight:700, cursor:'pointer', transition:'background 0.25s', boxShadow:'0 6px 24px rgba(245,200,66,0.3)' }}
